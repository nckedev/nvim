...
require("lazy").setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set("n", "<leader>fd", builtin.diagnostics, { desc = "[F]findj [D]iagnostics" })
      ...
      vim.keymap.set(
        ...
        { desc = "[g] Fuzzily search in current buffer" }
      )
  ...
  { -- Autocompletion
    ...
    config = function()
      ...
      cmp.setup({
        ...
        -- 	print("=================")
        -- 	print(vim.inspect(item.kind or ""))
        -- 	local maxlen = 30
        -- 	local minlen = 20
        -- 	local len = #item.abbr
        ...
        -- 	if len > maxlen then
        --
        -- 		item.abbr = vim.fn.strcharpart(content, 0, maxlen - 3) .. "..."
        -- 	end
        --
        -- 	if len < minlen then
        -- 		item.abbr = content .. (" "):rep(minlen - len)
        -- 	end
        --
        -- 	return item
        -- end,
